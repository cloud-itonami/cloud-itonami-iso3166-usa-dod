(ns dodcompliance.operation
  "OperationActor -- one DoD compliance operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (DoDCompliance-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the DoD Compliance Governor
  ...
  One graph run = one compliance operation (dfars / cmmc /
  facility-clearance track). No unbounded inner loop.
  ...
  governor is clean -- see `dodcompliance.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [dodcompliance.dodcompliancellm :as dodcompliancellm]
            [dodcompliance.governor :as governor]
            [dodcompliance.phase :as phase]
...
(defn build
  ...
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (dodcompliancellm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (dodcompliancellm/-advise advisor store request)]
            {:proposal p :audit [(dodcompliancellm/trace request p)]})))
